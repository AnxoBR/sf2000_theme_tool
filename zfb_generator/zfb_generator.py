...
class ZFBGenerator(tk.Frame):
    ...
    def create_interface(self):
        ...
        self.arcade_checkbutton = tk.Checkbutton(
            self, text="ARCADE", variable=self.arcade_var, command=self.toggle_arcade
        )
        ...
        self.create_zfb_button = tk.Button(
            self, text="Create ZFB Files", command=self.execute_create_zfb, font=("Helvetica", 14)
        )
        self.create_zfb_button.grid(row=6, column=0, columnspan=7, pady=10)

        # Mensajes de estado
        self.msg_var = tk.StringVar()
        tk.Label(self, textvariable=self.msg_var, font=label_font).grid(row=7, column=0, columnspan=7, sticky="w", padx=10)
    ...
    def create_label_entry(self, row, label_text, var_name, button_text, button_command, label_font):
        tk.Label(self, text=label_text, font=label_font).grid(row=row, column=0, sticky="w", padx=10)
        
        var = tk.StringVar()
        setattr(self, var_name, var)

        entry = tk.Entry(self, textvariable=var, width=70)
        setattr(self, f"{var_name}_entry", entry)
        entry.grid(row=row, column=1, columnspan=5, sticky="w", padx=5)

        button = tk.Button(self, text=button_text, command=button_command)
    ...
    def create_core_and_extension(self, label_font):
        ...
        self.core_label = tk.Label(self, text="CORE:", font=label_font)
        self.core_label.grid(row=3, column=0, sticky="w", padx=10)
        self.core_entry = tk.Entry(self, textvariable=self.core_var, width=10)
        self.core_entry.grid(row=3, column=1, sticky="w", padx=5)
    
        # EXTENSION
        self.extension_var = tk.StringVar()
        self.extension_label = tk.Label(self, text="EXTENSION:", font=label_font)
        self.extension_label.grid(row=3, column=2, sticky="w", padx=5)
        self.extension_entry = tk.Entry(self, textvariable=self.extension_var, width=10)
    ...
    def create_image_size_widgets(self, label_font):
        ...
        tk.Label(self, text="Image Size:", font=label_font).grid(row=5, column=0, sticky="w", padx=10)
        self.imgwidth_entry = tk.Entry(self, textvariable=self.imgwidth_var, width=10)
        self.imgwidth_entry.grid(row=5, column=1, sticky="w", padx=5)
        tk.Label(self, text="x", font=label_font).grid(row=5, column=2, sticky="w")
        self.imgheight_entry = tk.Entry(self, textvariable=self.imgheight_var, width=10)
        self.imgheight_entry.grid(row=5, column=3, sticky="w", padx=5)
    
        # Checkbox Fullscreen
        self.fullscreen_checkbox = tk.Checkbutton(
            self, text="Fullscreen", variable=self.img_fullscreen_var, command=self.toggle_fullscreen
        )
